...
fun SignInWithEmail(
    ...
    Column(
        ...
        Text("Age")
        TextField(value = state.age.toString(), onValueChange = { newValue -> onEvent(SignInAction.ChooseYearOfBirth(2025 - newValue.toInt()))}) // TODO: 2025
        Text("Email")
        ...
        Button(onClick = {onEvent(SignInAction.CreateUser(transitionToScreen))}) { Text("Create User") }
    }
}
...
fun SignInWithEmailPreview() {
    NAVITheme {
        SignInWithEmail(modifier = Modifier.fillMaxSize().padding(5.dp), onEvent = {}, state = SignInState(), transitionToScreen = {})
    }
}